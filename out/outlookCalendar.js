...
exports.signInToOutlook = signInToOutlook;
exports.signOutFromOutlook = signOutFromOutlook;
exports.isSignedInToOutlook = isSignedInToOutlook;
exports.fetchTodayOutlookMeetings = fetchTodayOutlookMeetings;
exports.startOutlookSync = startOutlookSync;
exports.stopOutlookSync = stopOutlookSync;
exports.getCachedOutlookMeetings = getCachedOutlookMeetings;
...
async function signInToOutlook(context) {
    try {
        ...
        const choice = await vscode.window.showInformationMessage(`📋 To sign in to Outlook, go to: ${deviceData.verification_uri}\n\nEnter code: ${deviceData.user_code}`, { modal: true }, 'Open Browser & Copy Code', 'Cancel');
        if (choice !== 'Open Browser & Copy Code') {
            return false;
        }
...
// ─── POLL FOR TOKEN ──────────────────────────────────────────
async function pollForToken(context, deviceCode, intervalSec) {
...
async function getAccessToken(context) {
    ...
    const refreshToken = context.globalState.get('outlookRefreshToken', '');
    if (!refreshToken) {
        return null;
    }
    try {
...
// ─── SIGN OUT ────────────────────────────────────────────────
async function signOutFromOutlook(context) {
...
// ─── CHECK IF SIGNED IN ──────────────────────────────────────
function isSignedInToOutlook(context) {
    const token = context.globalState.get('outlookAccessToken', '');
    const expiry = context.globalState.get('outlookTokenExpiry', 0);
    return !!(token && expiry > Date.now());
}
// ─── FETCH TODAY'S MEETINGS FROM GRAPH API ───────────────────
async function fetchTodayOutlookMeetings(context) {
    ...
    try {
        ...
        return context.globalState.get('outlookMeetings', []);
    }
}
// ─── START AUTO-SYNC EVERY 30 MINS ───────────────────────────
function startOutlookSync(context, onSync) {
    stopOutlookSync();
...
// ─── GET CACHED MEETINGS ─────────────────────────────────────
function getCachedOutlookMeetings(context) {
    return context.globalState.get('outlookMeetings', []);
}
function getLastSyncTime(context) {
    return context.globalState.get('outlookLastSync', 'Never');
}
//# sourceMappingURL=outlookCalendar.js.map