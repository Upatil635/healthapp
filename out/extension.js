...
const outlookCalendar_1 = require("./outlookCalendar");
// ─── GLOBAL STATE ───────────────────────────────────────────
let timers = [];
let meetings = [];
let isRunning = false;
...
function getSettings() {
    return globalContext.globalState.get('reminderSettings', {
...
class HealthSidebarProvider {
    ...
    resolveWebviewView(webviewView) {
        ...
        webviewView.webview.onDidReceiveMessage(async (message) => {
            switch (message.command) {
                ...
                // ── Outlook commands ──
                case 'outlookSignIn': {
                    const ok = await (0, outlookCalendar_1.signInToOutlook)(this._context);
                    if (ok) {
                        (0, outlookCalendar_1.startOutlookSync)(this._context, (mtgs) => {
                ...
                case 'outlookSignOut':
                    await (0, outlookCalendar_1.signOutFromOutlook)(this._context);
                    this.refresh();
                    break;
                case 'outlookSync': {
                    vscode.window.setStatusBarMessage('🔄 Syncing Outlook...', 2000);
                    const mtgs = await (0, outlookCalendar_1.fetchTodayOutlookMeetings)(this._context);
                    scheduleOutlookMeetings(this._context, mtgs);
                    this.refresh();
    ...
    getSidebarHTML() {
        ...
        const saved = globalContext.globalState.get('savedMeetings', []);
        const todayMeetings = saved.filter(m => m.date === today);
        const s = getSettings();
        const pad = (n) => String(n).padStart(2, '0');
        const dayStartVal = `${pad(s.dayStartHour)}:${pad(s.dayStartMin)}`;
        const snoozed = globalContext.globalState.get('snoozedReminders', []);
        const activeSnoozed = snoozed.filter(r => r.snoozeUntil > Date.now());
        const signedIn = (0, outlookCalendar_1.isSignedInToOutlook)(globalContext);
        const outlookMtgs = (0, outlookCalendar_1.getCachedOutlookMeetings)(globalContext);
        const lastSync = (0, outlookCalendar_1.getLastSyncTime)(globalContext);
...
HealthSidebarProvider.viewType = 'healthReminder.sidebar';
// ─── ACTIVATE ───────────────────────────────────────────────
function activate(context) {
    ...
    context.subscriptions.push(vscode.window.registerWebviewViewProvider(HealthSidebarProvider.viewType, sidebarProvider));
    context.subscriptions.push(vscode.commands.registerCommand('reminder.start', () => {
        if (!isRunning) {
        ...
        if (title && time) {
            scheduleMeeting(context, title, time);
            sidebarProvider.refresh();
        }
        ...
        const ok = await (0, outlookCalendar_1.signInToOutlook)(context);
        if (ok) {
            (0, outlookCalendar_1.startOutlookSync)(context, (mtgs) => {
        ...
        await (0, outlookCalendar_1.signOutFromOutlook)(context);
        sidebarProvider.refresh();
    }));
    // Auto-start health reminders
    const wasRunning = context.globalState.get('isRunning', true);
    if (wasRunning) {
        startReminders(context);
        isRunning = true;
        statusBarItem.text = '$(heart) Health ON';
    ...
    if ((0, outlookCalendar_1.isSignedInToOutlook)(context)) {
        (0, outlookCalendar_1.startOutlookSync)(context, (mtgs) => {
            scheduleOutlookMeetings(context, mtgs);
            sidebarProvider.refresh();
        });
...
function scheduleOutlookMeetings(context, mtgs) {
    const s = getSettings();
    mtgs.forEach(m => {
        ...
        if (diff > 5 * 60000) {
            setTimeout(() => {
                showPopup(context, `⏰ Meeting in 5 mins!`, `"${escapeHtml(m.title)}" starts at ${m.start}${m.joinUrl ? ' — Join link ready!' : ''}`, '#e67e22', s.snoozeMinutes, m.joinUrl);
            }, diff - 5 * 60000);
        }
        setTimeout(() => {
            showPopup(context, `📅 Meeting Now!`, `"${escapeHtml(m.title)}" is starting NOW!`, '#0078d4', s.snoozeMinutes, m.joinUrl);
        }, diff);
    });
}
// ─── RESTORE SNOOZED ────────────────────────────────────────
function restoreSnoozedReminders(context) {
    const snoozed = context.globalState.get('snoozedReminders', []);
    const now = Date.now();
    const stillPending = [];
    snoozed.forEach(r => {
...
function removeSnoozed(context, snoozeUntil) {
    const snoozed = context.globalState.get('snoozedReminders', []);
...
function stopReminders() {
    timers.forEach(t => clearInterval(t));
    timers = [];
    meetings.forEach(m => clearTimeout(m.timeout));
    meetings = [];
    (0, outlookCalendar_1.stopOutlookSync)();
}
// ─── MANUAL MEETINGS ────────────────────────────────────────
function deleteMeeting(index, context) {
    if (meetings[index]) {
...
function scheduleMeeting(context, title, time) {
    if (!title || title.length > 200) {
        vscode.window.showErrorMessage('❌ Title must be 1–200 chars');
        return;
    }
    if (!time || !/^\d{2}:\d{2}$/.test(time)) {
        vscode.window.showErrorMessage('❌ Time must be HH:MM');
        return;
    }
...
// ─── SHOW POPUP ─────────────────────────────────────────────
function showPopup(context, title, message, color, snoozeMinutes, joinUrl) {
    const panel = vscode.window.createWebviewPanel('healthReminder', title, vscode.ViewColumn.One, { enableScripts: true });
    panel.webview.html = getPopupHTML(title, message, color, snoozeMinutes, joinUrl);
    const autoClose = setTimeout(() => { try {
        panel.dispose();
    }
    catch (e) { } }, 30000);
...
// ─── POPUP HTML ─────────────────────────────────────────────
function getPopupHTML(title, message, color, snoozeMinutes, joinUrl) {
...
function deactivate() { stopReminders(); }
